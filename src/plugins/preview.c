...
static Preview * _preview_init(BrowserPluginHelper * helper)
{
	...
	/* audio */
	preview->view_audio = gtk_vbox_new(FALSE, 4);
	/* audio: progress bar */
	preview->view_audio_progress = gtk_hscale_new_with_range(0.0, 100.0,
			0.1);
	gtk_box_pack_start(GTK_BOX(preview->view_audio),
			preview->view_audio_progress, FALSE, TRUE, 0);
	gtk_widget_show(preview->view_audio_progress);
	...
	g_signal_connect_swapped(widget, "clicked",
			G_CALLBACK(_preview_on_audio_play), preview);
	...
	gtk_button_set_relief(GTK_BUTTON(widget), GTK_RELIEF_NONE);
	g_signal_connect_swapped(widget, "clicked",
			G_CALLBACK(_preview_on_audio_pause), preview);
	gtk_box_pack_start(GTK_BOX(hbox), widget, FALSE, TRUE, 0);
	/* audio: stop button */
	widget = gtk_button_new();
	gtk_button_set_image(GTK_BUTTON(widget),
	...
	gtk_button_set_relief(GTK_BUTTON(widget), GTK_RELIEF_NONE);
	g_signal_connect_swapped(widget, "clicked",
			G_CALLBACK(_preview_on_audio_stop), preview);
	gtk_box_pack_start(GTK_BOX(hbox), widget, FALSE, TRUE, 0);
	gtk_widget_show_all(hbox);
	gtk_box_pack_start(GTK_BOX(preview->view_audio), hbox, FALSE, TRUE, 0);
	gtk_widget_set_no_show_all(preview->view_audio, TRUE);
	gtk_box_pack_start(GTK_BOX(vbox), preview->view_audio, FALSE, TRUE, 0);
	/* image */
...
static void _preview_refresh(Preview * preview, GList * selection)
{
	...
	if(strcmp(type, audio) == 0)
		preview->source = g_idle_add(_preview_on_idle_audio, preview);
	else if(strncmp(type, image, sizeof(image)) == 0)
...
static void _refresh_reset(Preview * preview)
{
	...
	gtk_widget_hide(preview->view_audio);
...
static gboolean _preview_on_idle_audio(gpointer data)
{
	...
	preview->source = 0;
	/* FIXME implement */
	gtk_widget_show(preview->view_audio);
	return FALSE;
}
